...
function GetAbsoluteValidation(
  ...
  return (
    max_req_fee <= fee * GWEI_TO_WEI &&
    max_req_priority <= priority * GWEI_TO_WEI
  );
}

async function GetPendingsHandler(
  fileText: string,
  rpc: string,
  setWallets: Function,
  setPendingTable: Function
) {
  ...
  await GetPendings(privateKeys, rpc, setWallets, setPendingTable);
}
...
function MainForm(props: any) {
  ...
  const [rpc, setRPC] = useState(LOADED_RPC);
  const [speedupType, setSpeedupType] = useState(SpeedupType.Absolute);
  const [wallets, setWallets] = useState(new Array<any>());
  return (
    <div>
      <form>
        {speedupType == SpeedupType.Absolute && (
          <div className="flex mb-6 w-full">
            <div className="flex w-full">
              <FaGasPump
                color={props.pendingTable.length > 0 ? 'red' : 'gray'}
                size={'3em'}
                className="mt-5 mr-5"
              />
            ...
            <div className="flex ml-6 w-full">
              <MdPriorityHigh
                color={props.pendingTable.length > 0 ? 'green' : 'gray'}
                size={'3em'}
                className="mt-5 mr-5"
              />
              <div className="relative z-0 w-full group">
        ...
        {speedupType == SpeedupType.Percent && (
          <div className="flex mb-6 w-full">
            <FaPercentage
              color={props.pendingTable.length > 0 ? 'blueviolet' : 'gray'}
              size={'3em'}
              className="mt-5 mr-5"
            />
            <div className="relative z-0 w-full group">
          ...
          <div className="flex items-center mb-4">
            <input
              ...
              onChange={() => {
                ...
                else setSpeedupType(SpeedupType.Percent);
              }}
              value=""
      ...
      <div className="flex mb-4">
        ...
        <button
          ...
          disabled={
            !(
              props.pendingTable.length > 0 &&
              ((speedupType == SpeedupType.Absolute &&
                GetAbsoluteValidation(props.pendingTable, fee, priority)) ||
                (speedupType == SpeedupType.Percent && percent >= 10))
            )
          }
          onClick={() => {
            SpeedUp(wallets, props.pendingTable, props.setPendingTable, {
              type: speedupType,
              priority: priority,
              fee: fee,
              percent: percent,
