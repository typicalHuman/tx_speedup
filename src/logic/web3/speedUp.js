...
export async function SpeedUp(
  ...
  if (speedupInfo['type'] == SpeedupType.Absolute) {
    await SpeedupAbsolute(
      wallets,
      pendingTable,
      setPendingTable,
      speedupInfo['fee'],
      speedupInfo['priority']
    );
  } else {
    await SpeedupPercent(
      wallets,
      pendingTable,
      setPendingTable,
      speedupInfo['percent']
...
async function SpeedupAbsolute(
  wallets,
  pendingTable,
  setPendingTable,
  fee,
  ...
  for (let i = 0; i < pendingTable.length; i++) {
    ...
    pendingTable[i]['maxPriorityFeePerGas'] = BigNumber.from(
      priority * GWEI_TO_WEI
    );
  }
...
async function SpeedupPercent(wallets, pendingTable, setPendingTable, percent) {
  for (let i = 0; i < pendingTable.length; i++) {
    ...
    pendingTable[i]['maxFeePerGas'] = BigNumber.from(
      fee + (fee / 100) * percent
    );
    pendingTable[i]['maxPriorityFeePerGas'] = BigNumber.from(
      priority + (priority / 100) * percent
    );
  }
  console.log(pendingTable);
  await ProcessTransactions(wallets, pendingTable, setPendingTable);
...
async function ProcessTransactions(wallets, txs, setPendingTable) {
  ...
  for (let i = 0; i < current_txs.length; i++) {
    try {
      var wallet = wallets.filter(
        (w) =>
          w['address'].toLowerCase() == current_txs[i]['from'].toLowerCase()
      )[0]['wallet'];
